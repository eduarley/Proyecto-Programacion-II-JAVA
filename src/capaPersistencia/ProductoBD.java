...
public class ProductoBD {
    private final String RUTA_ARCHIVO = System.getProperty("user.dir").replace("\\dist", "")+ "\\src\\Archivo\\Producto.txt";  //Importante poner doble slash
   ...
   //parámetro, en caso de que no lo encuentre retorna null
    public Producto consultarProducto(String codigoProd)throws Exception {
        ...
        try {
            ...
            //Si no hay más datos que leer el método available retorna cero
             while(true){
                ...
                if(prod.getCodigo().equalsIgnoreCase(codigoProd)) {
                    prodBuscado = prod;
                }
             }            
        }catch(Exception e){
     ...
     * @param prod Objeto Departamento a agregar
     * @return void
     */
    public  void agregarProducto(Producto prod)throws Exception {        
        try {
            this.abrirArchivoOutput();
            //if (oEscritor != null) {
              //Ejecutar la escritura del objeto pDepartamento en el archivo
               oEscritor.writeObject(prod);
        ...
        try {            
            ...
            //Si no hay más datos que leer el método available retorna cero
             while(true){//Si va a leer y no hay objeto Departamento se va por el catch
                 prod1 = (Producto)oLector.readObject(); 
                 if(prod1.getCodigo().equalsIgnoreCase(producto.getCodigo())) {
                   prod1=producto;
                 }
                 arrayProductoTemp.add(prod1);
             }  
        }
        catch(Exception ex){
    ...
    public void eliminarProducto(String codigoDepto) throws Exception {
        arrayProductoTemp = new ArrayList<Producto>();
        try {            
            abrirArchivoInput();
            Producto prod = null;
            ...
            while(true){
                 prod = (Producto)oLector.readObject();               
                 if(!prod.getCodigo().equalsIgnoreCase(codigoDepto)) {
    ...
    private void pasarArrayTemporal_Archivo() throws Exception {       
       ...
       //borra el archivo original para volverlo a llenar           
          ...
          //Pasa todos los departamentos del ArrayList al archivo
            for (Producto prod : arrayProductoTemp) {
             oEscritor.writeObject(prod);       
            }     
          }
          cerrarArchivoOutput();
